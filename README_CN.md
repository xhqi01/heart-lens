...
> **隐私模型。** v1 完全运行在浏览器中；v2 将数据存储在 **你自己服务器的数据库** 里，所有模型调用都在服务端代理。除了你自己配置的大模型服务商之外，数据不会发往任何第三方。
...
Next.js 14（App Router，TypeScript）· Prisma + SQLite · `jose` 会话 · `bcryptjs` · AES-256-GCM · `zod`（输入校验）+ `ajv`（模型输出校验）· Vitest。
